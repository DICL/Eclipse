...
// for ATC
#define IBUFFERTHRESHOLD (4*1024)
...
msgaggregator::msgaggregator()
{
    ...
    dwriter = NULL;
    dwritepos = 0;
    // message = (char*) malloc(READAHEAD_SIZE);
    message = (char*)malloc(BUF_SIZE);
    // diskread_message = (char*)malloc(READAHEAD_SIZE);
...
// bool msgaggregator::add_diskread_record (string* record) {
// // return true when flushed
//     // check whether the new record can be added
//     if (READAHEAD_SIZE - pos - 2 > record->length()) {
//     // new record can be added
//         // add newline character at the end of current message
//         if ( (unsigned) pos != initial.length()) {
//             diskread_message[pos] = '\n';
//             pos++;
//         }
//         
//         // append the content(record)
...
//         }
//     } else {
//     // new record should be added to next buffer and current buffer should be flushed
//         flush();
//         //message[pos] = '\n';
//         //pos++;
//         // append the content(record)
...
memset(Imessage, 0, IBUFFERSIZE);
    ...
    // memset (message, 0, READAHEAD_SIZE);
...
// void msgaggregator::flush_diskread() {
// // return false when new messagebuffer is needed to be created
//     // do not flush when message have no information
//     if ( (unsigned) pos == initial.length()) {
//         return;
//     }
...
//     // write to the fd
//     if (msgbuf == NULL) {
//     // no target messagebuf(client side)
// //cout<<"flushed message: "<<message<<endl<<endl;
//         nbwrite (fd, message);
//         // set initial contents
//         set_initial();
//         return;
//     } else {
//     // write to target messagebuf(fileserver side)
//         if (msgbuf->size() > 1) {
//             //cout<<"flushed message: "<<message<<endl<<endl;
//             msgbuf->back()->set_buffer (message, fd);
//             msgbuf->push_back (new messagebuffer());
