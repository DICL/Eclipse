...
gettimeofday(&(jobs[jobs.size() - 1]->job_start), NULL);
			}
			else
			{
...
if(thejob->get_numcompleted_tasks()%100 == 0)
...
gettimeofday(&(jobs[i]->map_end), NULL);
				...
				{
gettimeofday (&(jobs[i]->job_end), NULL);
...
if(jobs[i]->getjobid() == 24)
{
cout << "----------scheduled counts---------" << endl;
for(int j = 0; j < 38; j++)
{
	cout << scheduledcount[j] << endl;
}
cout << "-----------------------------------" << endl;
}

double map_elapsed = 0;
double job_elapsed = 0;

map_elapsed = jobs[i]->map_end.tv_sec - jobs[i]->job_start.tv_sec;
map_elapsed += ((double)jobs[i]->map_end.tv_usec - (double)jobs[i]->job_start.tv_usec)/1000000.0;

job_elapsed = jobs[i]->job_end.tv_sec - jobs[i]->job_start.tv_sec;
job_elapsed += ((double)jobs[i]->job_end.tv_usec - (double)jobs[i]->job_start.tv_usec)/1000000.0;

cout << "\t" << "[" << jobs[i]->getargvalue(0) << "]" << "map elapsed: " << map_elapsed << endl;
cout << "\t" << "[" << jobs[i]->getargvalue(0) << "]" << "job elapsed: " << job_elapsed << endl;
		...
		for ( int i = 0; (unsigned) i < slaves.size(); i++ ) {
			...
			for ( int j = 0; (unsigned) j < jobs.size(); j++ ) {
				master_task* thetask = jobs[j]->get_lastwaitingtask();
				...
				if (thetask->get_taskrole() == MAP) {
					...
					if ( (slaves[i]->getnumrunningtasks() < slaves[i]->getmaxmaptask())
						&& (thehistogram->get_index (hash_value) == i || jobs[j]->getdelayed() > delayThreashold) ) // local
					{
//						cout << jobs[j]->getdelayed() << "M NODE " << i << ": JOB " << jobs[j]->getjobid() << "-" << thetask->gettaskid() << " is scheduled. " << endl;
						jobs[j]->sched();
						stringstream ss;
						ss << "tasksubmit ";
						ss << jobs[j]->getjobid();
...
scheduledcount[i] = scheduledcount[i] + 1;
						// sleeps for 0.0001 seconds. change this if necessary
						// usleep(100000);
						break;
					} else if (slaves[i]->getnumrunningtasks() < slaves[i]->getmaxmaptask()) {
//						cout << jobs[j]->getdelayed() << endl;; 
						//jobs[j]->delayed((int)std::time(NULL));
						jobs[j]->delayed();
					}
				} else {
					int nodeidx = thetask->peerids[0];
					if ( nodeidx == i && slaves[i]->getnumrunningtasks() < slaves[i]->getmaxreducetask() )
					{
//						cout << "R NODE " << i << ": JOB " << jobs[j]->getjobid() << "-" << thetask->gettaskid() << " is scheduled. " << endl;
						jobs[j]->sched();
						stringstream ss;
						ss << "tasksubmit ";
						ss << jobs[j]->getjobid();
						...
						for (int k = 0; (unsigned) k < thetask->peerids.size(); k++)       // don't need to worry about BUF_SIZE overflow in reducer case
						{
							stringstream ss;
							ss << " ";
							ss << thetask->peerids[k];
							ss << " ";
							ss << thetask->numiblocks[k];
...
scheduledcount[j] = scheduledcount[j] + 1;
					// sleeps for 0.0001 seconds. change this if necessary
					// usleep(100000);
				}
			}
		...
		for (int iter = 0; (unsigned) iter < jobs.size(); iter++)
		{
			...
			for (int k = 0; k < jobs[i]->get_numwaiting_tasks(); k++)
			{
				...
				if (thetask->get_taskrole() == MAP)
				{
					...
					if (slaves[nodeindex]->getnumrunningtasks() >= slaves[nodeindex]->getmaxmaptask())
					{
						continue;
					}
...
scheduledcount[nodeindex] = scheduledcount[nodeindex] + 1;
