...
bool socket_thread_continue;
bool mpi_thread_continue;
...
int main (int argc, char** argv)
{
	...
	while(1) // connect slaves using MPI
	{
		int slaverank; // slave's rank

		//blocking receive from slaves or jobs
		...
		{
			// using mpi, slaves have no ip address. Therefore, rank also sotred when slave pushed 
			connslave tmpslave (slaverank);
			tmpslave.setmaxmaptask (MAP_SLOT);
			tmpslave.setmaxreducetask (REDUCE_SLOT);
			slaves.push_back (tmpslave);
			connectioncount++;
			cout << "a slave node connected from rank " << therank << endl;
		}
		else
		{
	...
	socket_thread_continue = true;
	mpi_thread_continue = true;
	pthread_t listener_thread;
	pthread_create (&listener_thread, NULL, socket_listener, (void*) &serverfd);
	pthread_create (&listener_thread, NULL, mpi_listener, (void*) &masterrank);

	// sleeping loop which prevents process termination
	while (socket_thread_continue==true || mpi_thread_continue==true)
	{
		sleep (1);
	}
...
void* socket_listener (void* args) // for client and ipc network
{
...
void* mpi_listener (void* args)
{
	// mpi network
	int serverrank = * ( (int*) args);
	...
	// listen signals from slave nodes and jobs
	while (1)
	{
		// blocking read to check identification of connected node
							}
...
void* mpi_listener (void* args)
{
	...
	while (1)
	{
		...
		if (tmprank >= 0)
		{
			if (readbytes == 0)
			{
				cout << "[master]Connection closed before getting message" << endl;
				tmprank = -1;
				continue;
			}
			else if (strncmp (read_buf, "slave", 5) == 0)
			{
				...
				memset (write_buf, 0, BUF_SIZE);
				strcpy (write_buf, "close");
				MPI_Isend(&write_buf, BUF_SIZE, MPI_CHAR, tmprank, 0, MPI_COMM_WORLD, request);
				MPI_Wait(&request, &status);
			}
			else if (strncmp (read_buf, "job", 3) == 0)         // if connected node is a job
			{
		...
		{
			MPI_Irecv(&read_buf, BUF_SIZE, MPI_CHAR, slaves[i]->getrank(), 0, MPI_COMM_WORLD, request);
			MPI_Wait(&request, &status);
			MPI_Get_count(&status, MPI_CHAR, &readbytes);
