...
class fileserver   // each slave node has an object of fileserver
{
    private:
		...
		//vector<int> write_finish_cnt;
		//vector<bool*> is_write_finish;
        

		//bool is_set_write_finish_var;
...
fileserver::fileserver()
{
	...
	//is_set_write_finish_var = false;


//read_buf = new char[IBUFFERSIZE];
//write_buf = new char[IBUFFERSIZE];
}
...
//				strcpy(write_buf, read_buf);
				...
				{
// !!!_20 Find the cache server location with inputpath(block_name)
// cachekey is used for retrieving data from selected cache server.
					string cachekey;
					string appname;
					int jobid;
...
//								cout<<"\033[0;31mLocal Cache miss\033[0m"<<endl;
							...
							else // when the data is in the cache
							{
								...
								// cout<<"\033[0;31mLocal Cache hit\033[0m"<<endl;
								if(theentry->is_being_written())
								{
                  cout << "Hit but being written" << endl;
									// determine the DHT file location
									hashvalue = hashvalue%nodelist.size();
...
//cout<<"[fileserver:"<<networkidx<<"]Iread message: "<<read_buf<<endl;
					...
					token = strtok(NULL, " "); // <- [hash value]
					hash_value = atoi(token);

					token = strtok(NULL, " "); // <- [num iblock]
					...
					if(peerid == networkidx) // local
					{
						...
						ireader* thereader = new ireader(jobid, numiblock, peerid, hash_value, 0, CLIENT);
						thereader->set_dstclient(clients[i]);
						ireaders.push_back(thereader);
						clients[i]->SetIreaderIndex(ireaders.size()-1);
					}
					else // remote
					{
//cout << "[fileserver]Iread request to read remote iData" << endl;
...
//cout<<"MWwrite"<<endl;
				...
				else if(strncmp(read_buf, "RWwrite", 7) == 0)
				{
				...
				else if(strncmp(read_buf, "OCwrite", 7) == 0) {
					string record;
					...
					/*
  					Appended part: Write the rusult of reducers on cache
  					*/
					...
					if (thewriter != NULL) {
					...
					else if (thepeer != NULL) {
						string message = "Dcache ";
						...
						else if (nbwritebuf(thepeer->get_fd(), write_buf, thepeer->msgbuf.back()) <= 0) {
							thepeer->msgbuf.push_back(new messagebuffer());
						}
					}
					else {
						...
						if (index == networkidx) {
							...
							thepeer = peers[index];
							clients[i]->set_Itargetpeer(thepeer);
							clients[i]->set_Icachekey(filename);
				...
				{
					cout<<"[fileserver:"<<networkidx<<"]Debugging: Unknown message" << endl;
				}
...
//							cout<<"\033[0;31mRemote Cache miss\033[0m"<<endl;
							...
							if(networkidx == index) // local DHT file
							{
								...
								thebridge->writebuffer->set_dwriter(thewriter);
								thebridge->writebuffer->set_dwritepos(bridgeid.length());
...
//cout<<"[fileserver:"<<networkidx<<"]reading file name: "<<filename<<endl;
				...
				else if(strncmp(read_buf, "Iread", 5) == 0)
				{
					...
					ireader* thereader = new ireader(jobid, numiblock, i, hash_value, bridgeid, PEER);
...
//cout<<"[fileserver:"<<networkidx<<"]Cachekey of written data: "<<cachekey<<endl;
				...
				else if(strncmp(read_buf, "Dcache", 6) == 0) {
//cout<<"[fileserver:"<<networkidx<<"]Icache message network idx: "<<networkidx<<endl;
					char* token;
					int bridgeindex;
					filebridge* thebridge;
...
//cout<<"[fileserver:"<<networkidx<<"]Edist arrived. notifying client end of distribution..."<<endl;
				...
				else if(strncmp(read_buf, "Iskip", 5) == 0) {
					cout << "Iskip arrived in datastream." << endl;
				}
				else // a filebridge id is passed. this is the case of data read stream
				{
					filebridge* thebridge = NULL;
					string record;
					char* token;
					char* buf;
					int id;
...
//cout << "[fileserver]Record from cache: " << record << endl;
//sleep(1);
...
//cout << "[fileserver]Record from cache: " << message << endl;
//sleep(1);
...
//cout << "[fileserver]Sending record: " << record << endl;
...
//gettimeofday(&time_start2, NULL);
		...
		for(int i = 0; (unsigned)i < iwriters.size(); i++) {
			for (int j = 0; j < REDUCE_SLOT; ++j) {
				if ((iwriters[i]->is_writing(j)) && (!iwriters[i]->IsWriteFinish(j)))
				{
					//bool ret;

					iwriters[i]->write_to_disk(j);
				}
			}

			if(iwriters[i]->IsFinish()) // writing finished
			{
		...
		for(int i = 0; (unsigned)i < peers.size(); i++)
		{
			...
			{
//int length = peers[i]->msgbuf.front()->get_message().length();
//int memsetsize = BUF_CUT*(length/BUF_CUT + 1);
//memset(write_buf, 0, memsetsize);
				memset(write_buf, 0, BUF_SIZE);
		...
		for(int i = 0; (unsigned)i < clients.size(); i++)
		{
			while(clients[i]->msgbuf.size() > 1)
			{
				...
				{
//int length = clients[i]->msgbuf.front()->get_message().length();
//int memsetsize = BUF_CUT*(length/BUF_CUT + 1);
//memset(write_buf, 0, memsetsize);
					memset(write_buf, 0, BUF_SIZE);
